...
def connectToSQLiteDB( dbname ):
    ...
    with con:
        try:
            ...
            cur.execute('CREATE TABLE IF NOT EXISTS "MetaInfo" ("VarName" NVARCHAR PRIMARY KEY  NOT NULL  UNIQUE , "VarValue" VarChar)')
        except:
            pass
...
def getUpdateLinkFromDB():
    ...
    with con:
        try:
            ...
            row = cur.fetchone()
            if row == None:
                cur.execute("INSERT OR IGNORE INTO MetaInfo (VarName, VarValue) VALUES ('NextLink','http://api.rainchasers.com/v1/river')");
